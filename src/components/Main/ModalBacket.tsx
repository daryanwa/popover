...
const ModalBacket = () => {
  ...
  const price = () => {
    return bucketItems.reduce((sum: number, item: DataMenuItems) => sum + item.price * item.count, 0);
  };

  const totalPos = () => {
    return bucketItems.reduce((total: number, item: DataMenuItems) => total + item.count, 0);
  };
    ...
    <div className={style.modalContainer}>
      ...
      {bucketItems.length <= 0 ? 'Bucket is empty' : (
        <div style={{ display: 'flex', flexFlow: 'column', justifyContent: 'center', alignItems: 'center' }}>
          ...
          <p style={{ fontSize: '2vh' }}>Price: <span style={{ fontWeight: 'bold' }}>{price()} $</span></p>
